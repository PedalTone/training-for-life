...
export async function createTrainingInsights(apiKey: string, sessions: InsightSession[], periodDays: number) {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: `Review this ${periodDays === 0 ? "all-history" : `${periodDays}-day`} training log. Completed workouts, notes, workout details, mobility completion, effort, and injury reports may all be relevant. Return useful progress insights and a short next-step plan.\n\n${JSON.stringify(compactSessions)}`,
