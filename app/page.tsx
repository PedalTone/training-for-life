...
function HistoryView({ now, sessions, activeSchedule, scheduleHistory, onOpenDate }: { now: Date; sessions: Session[]; activeSchedule: Schedule; scheduleHistory: ScheduleSnapshot[]; onOpenDate: (date: Date) => void }) {
  ...
  return <div className="subpage history-page">
    <div className="history-controls"><div className="segmented"><button className={view === "weeks" ? "active" : ""} onClick={() => setView("weeks")}>Weekly Details</button><button className={view === "month" ? "active" : ""} onClick={() => setView("month")}>Month</button></div><div className="history-period-nav" aria-label={view === "month" ? `${historyCursor.toLocaleDateString("en-US", { month: "long", year: "numeric" })} navigation` : `${weekDates(historyCursor)[0].toLocaleDateString("en-US", { month: "short", day: "numeric" })} history navigation`}><button onClick={() => shiftHistory(-1)} aria-label={view === "month" ? "View previous month" : "View previous three weeks"}>‹</button>{view === "month" && <><span className="history-nav-divider" aria-hidden="true"/><button onClick={() => shiftHistory(1)} disabled={currentPeriod} aria-label="View next month">›</button></>}</div><button className={flaggedOnly ? "filter active" : "filter"} onClick={() => setFlaggedOnly(!flaggedOnly)}>✚ Notables</button></div>
    ...
    {view === "weeks" && !flaggedOnly && !currentPeriod && <button className="history-next-period" onClick={() => shiftHistory(1)} aria-label="View next three weeks">Next 3 weeks <span aria-hidden="true">›</span></button>}
  </div>;
}
