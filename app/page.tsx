...
export default function Home() {
  ...
  const applyScreenshot = () => {
    ...
    const firstImport = importedWorkouts.length === 1;
      ...
      {tab === "today" && <div className="today-page">
        ...
        <details className="surface-card log-workout-card" open={openPanel === "log"} onToggle={(e) => togglePanel("log", e.currentTarget.open)}>
          ...
          <div className="log-workout-body">
            ...
            <section className="log-subsection data-subsection"><div className="log-subsection-heading"><span>▤</span><div><b>Workout data</b><small>Type it in or import up to six screenshots</small></div></div><section className="screenshot-import"><div className="screenshot-import-title"><div><strong>Track sessions from screenshots</strong><small>Add each track session separately; each one is saved without replacing the last.</small></div><b className="screenshot-count" aria-label={`${session.importedWorkouts?.length ?? 0} of 6 sessions imported`}>{session.importedWorkouts?.length ?? 0} / 6 <span>sessions</span></b></div>{!hasScreenshotAccess && <div className="screenshot-access-note"><span>AI access is managed in Settings.</span><button onClick={() => navigate("more")}>Open Settings</button></div>}<div className="screenshot-actions"><button onClick={() => void pasteScreenshot()} disabled={screenshotState === "reading" || (session.importedWorkouts?.length ?? 0) >= 6}>{session.importedWorkouts?.length ? "Paste next screenshot" : "Paste screenshot"}</button><button onClick={() => screenshotInput.current?.click()} disabled={screenshotState === "reading" || (session.importedWorkouts?.length ?? 0) >= 6}>Choose from Photos</button></div><input ref={screenshotInput} type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void readScreenshot(file); event.currentTarget.value = ""; }}/>{screenshotState === "reading" && <p className="screenshot-status" role="status"><span/>Reading workout details…</p>}{screenshotError && <p className="screenshot-error" role="alert">{screenshotError}</p>}{Boolean(session.importedWorkouts?.length) && <div className="imported-workout-list" aria-label="Imported workout sessions">{session.importedWorkouts?.map((workout, index) => <div className="imported-workout-row" key={`${workout.date}-${workout.startTime}-${index}`}><div><b>Session {index + 1}</b><span>{[workout.date, workout.startTime, workout.duration, workout.distance, workout.pace].filter(Boolean).join(" · ") || "Details imported"}</span></div><button className="text-button" onClick={() => removeImportedWorkout(index)}>Remove</button></div>)}</div>}</section><div className="field-grid"><label><span>Duration</span><div><input value={session.duration} onChange={(e) => update({ duration: e.target.value })} placeholder="—"/></div></label><label><span>Distance</span><div><input value={session.distance} onChange={(e) => update({ distance: e.target.value })} placeholder="—"/></div></label><label><span>Pace</span><div><input value={session.pace ?? ""} onChange={(e) => update({ pace: e.target.value })} placeholder="—"/></div></label><label><span>Calories</span><div><input inputMode="numeric" value={session.calories ?? ""} onChange={(e) => update({ calories: e.target.value })} placeholder="—"/></div></label><label><span>Start time</span><div><input value={session.startTime ?? ""} onChange={(e) => update({ startTime: e.target.value })} placeholder="—"/></div></label></div><div className="effort-row"><span>Perceived effort</span><div>{(["easy", "moderate", "hard"] as Effort[]).map((effort) => <button key={effort} className={session.effort === effort ? "selected" : ""} onClick={() => update({ effort: session.effort === effort ? "" : effort })}>{effort}</button>)}</div></div>{session.detailSource && <p className="detail-source">Imported from {session.detailSource} · You can edit any value.</p>}</section>
