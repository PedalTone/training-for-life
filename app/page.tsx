...
function HistoryView({ now, sessions, activeSchedule, insightReports, setInsightReports, fitnessGoals, onOpenDate }: { now: Date; sessions: Session[]; activeSchedule: Schedule; insightReports: TrainingInsightReport[]; setInsightReports: React.Dispatch<React.SetStateAction<TrainingInsightReport[]>>; fitnessGoals: FitnessGoals; onOpenDate: (date: Date) => void }) {
  ...
  const weekBlocks = Array.from({ length: 4 }, (_, w) => { const date = new Date(historyCursor); date.setDate(historyCursor.getDate() - w * 7); return weekDates(date); });
  ...
  const shiftHistory = (direction: number) => setHistoryCursor((current) => { const next = new Date(current); if (view === "month") next.setMonth(next.getMonth() + direction); else next.setDate(next.getDate() + direction * 28); return next; });
