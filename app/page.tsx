      ...
      {tab === "today" && <div className="today-page">
        ...
        <details className="surface-card log-workout-card" open={openPanel === "log"} onToggle={(e) => togglePanel("log", e.currentTarget.open)}>
          ...
          <div className="log-workout-body">
            <details className="log-subsection note-subsection" open={Boolean(session.notes)}><summary className="log-subsection-heading"><span>✎</span><div><b>Note</b><small>Dictate important details about your workout</small></div><i>＋</i></summary><textarea value={session.notes} onChange={(e) => update({ notes: e.target.value })} placeholder="Add workout note…" rows={7} aria-label="Workout note"/></details>
