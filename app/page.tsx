    ...
    <main>
      {tab === "today" && <div className="today-page">
        ...
        <div className="control-row workout-mobility-row today-primary-actions">
          ...
          <button className={`mobility-loader ${showMobilityPicker ? "active" : ""}`} onClick={openMobilityPicker} aria-expanded={showMobilityPicker}><span>↗</span><b>Add-ons</b><small>{session.mobilityExercises.length ? `${session.mobilityExercises.length} selected` : "Choose supporting work"}</small></button>
        </div>
        ...
        <details className="surface-card log-workout-card" open={openPanel === "log"} onToggle={(e) => togglePanel("log", e.currentTarget.open)}>
          ...
          <div className="log-workout-body">
            <section className="log-subsection note-subsection"><div className="log-subsection-heading"><span>✎</span><div><b>Note</b><small>Add what matters</small></div></div><div className="note-meta"><span className={`save-pill ${saveState === "Saving…" ? "saving" : ""}`}>● {saveState}</span></div><textarea value={session.notes} onChange={(e) => update({ notes: e.target.value })} placeholder="Add workout note…" rows={3} aria-label="Workout note"/><p>Use the iPhone keyboard microphone to dictate.</p></section>
            ...
            <section className="log-subsection links-subsection"><div className="log-subsection-heading"><span>▶</span><div><b>YouTube links</b><small>{session.videos.length ? `${session.videos.length} saved` : "Add a workout video"}</small></div></div><div className="inline-sheet"><input type="url" value={videoUrl} onChange={(e) => { videoLabelEdited.current = false; setVideoUrl(e.target.value); setVideoLabel(""); setVideoMessage(""); }} placeholder="Paste YouTube URL"/><input aria-label="YouTube video label" value={videoLabel} onChange={(e) => { videoLabelEdited.current = true; setVideoLabel(e.target.value); }} placeholder="Video title loads automatically"/><button className="compact-primary" onClick={attachVideo} disabled={attachingVideo}>{attachingVideo ? "Saving…" : "Save video + build guide"}</button>{videoMessage && <p className="video-message" role="status">{videoMessage}</p>}</div><div className="video-grid">{session.videos.map((video, i) => <VideoCard video={video} onDelete={() => deleteVideo(session.id, i)} onClearGuide={video.workoutGuide ? () => clearWorkoutGuide(session.id, i) : undefined} onRetry={() => analyzeVideo(i, video)} key={`${video.url}-${i}`}/>)}</div></section>
...
function MobilityPicker({ exercises, selected, completed, sessions, currentDate, toggleExercise, toggleCompleted, onDone, onCancel }: { exercises: LibraryExercise[]; selected: string[]; completed: string[]; sessions: Session[]; currentDate: string; toggleExercise: (name: string) => void; toggleCompleted: (name: string) => void; onDone: () => void; onCancel: () => void }) {
  ...
  return <div className="mobility-sheet-backdrop" onClick={onCancel}><section className="mobility-sheet" role="dialog" aria-modal="true" aria-labelledby="mobility-sheet-title" onClick={(e) => e.stopPropagation()}><div className="mobility-sheet-header"><div><span className="kicker">ADD-ONS · {selected.length} SELECTED</span><h2 id="mobility-sheet-title">Choose and check exercises</h2><p>Selected exercises stay at the top so you can check them off in the same window.</p></div><button onClick={onCancel} aria-label="Close add-ons">×</button></div><div className="mobility-search"><span aria-hidden="true">⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search exercises" aria-label="Search mobility exercises"/></div><div className="library-list mobility-sheet-list">{filtered.map(({ id, name, equipment, added, done, lastDone, graphicData }) => { const historyLabel = lastDone ? `Last done ${dateFromKey(lastDone).toLocaleDateString("en-US", { month: "short", day: "numeric" })}` : "Not done yet"; return <div key={id} className={`mobility-picker-row ${added ? "added" : ""} ${done ? "checked" : ""}`}><button className="mobility-picker-select" aria-pressed={added} onClick={() => toggleExercise(name)}><span className="exercise-visual"><MovementMark exerciseId={id} name={name} graphicData={graphicData}/></span><span><strong>{name}</strong><small>{equipment}</small><small className="mobility-history">{historyLabel}</small></span><em>{added ? "Selected" : "+ Add"}</em></button><button className="mobility-picker-check" aria-label={`${done ? "Uncheck" : "Check off"} ${name}`} aria-pressed={done} disabled={!added} onClick={() => toggleCompleted(name)}><span>{done ? "✓" : ""}</span><small>Done</small></button></div>; })}{filtered.length === 0 && <p className="empty-state">No exercises match that search.</p>}</div><div className="mobility-sheet-footer"><span>{selected.length} selected · {selected.filter((name) => completed.includes(name)).length} done</span><button onClick={onDone}>Done</button></div></section></div>;
}
...
function MoreView({ libraryExercises, setLibraryExercises, futureVideos, setFutureVideos, insightReports, setInsightReports, fitnessGoals, setFitnessGoals, scheduleKeys, setScheduleKeys, sessions, setHistory, aiAccessCode, onSaveAiAccessCode, onDeleteVideo, onClearGuide, onAddToToday }: { libraryExercises: LibraryExercise[]; setLibraryExercises: React.Dispatch<React.SetStateAction<LibraryExercise[]>>; futureVideos: Video[]; setFutureVideos: React.Dispatch<React.SetStateAction<Video[]>>; insightReports: TrainingInsightReport[]; setInsightReports: React.Dispatch<React.SetStateAction<TrainingInsightReport[]>>; fitnessGoals: FitnessGoals; setFitnessGoals: React.Dispatch<React.SetStateAction<FitnessGoals>>; scheduleKeys: string[]; setScheduleKeys: React.Dispatch<React.SetStateAction<string[]>>; sessions: Session[]; setHistory: React.Dispatch<React.SetStateAction<Session[]>>; aiAccessCode: string; onSaveAiAccessCode: (value: string) => void; onDeleteVideo: (sessionId: string, videoIndex: number) => void; onClearGuide: (sessionId: string, videoIndex: number) => void; onAddToToday: (video: Video) => Promise<string> }) {
  ...
  return <div className="subpage more-page">
    ...
    <details className="settings-card about-card"><summary className="about-summary"><span className="setting-icon data">i</span><span><strong>About</strong><small>Privacy, version, and important information</small></span><i>＋</i></summary><div className="privacy-card"><span>PRIVATE BY DEFAULT</span><h2>Your history stays under your control.</h2><p>No analytics and no automatic workout uploads. Workout history stays on this device unless you request an AI feature. A screenshot is sent only when you choose Paste or Photos, and the image is discarded after the review; only values you apply are saved and backed up. Compact history fields are sent when you generate AI insights. Video links are sent only when you request a workout guide.</p><p className="disclaimer">This is a tracking tool, not medical advice. Use controlled movement and an appropriate load; stop for sharp pain and seek qualified care when needed.</p><p className="about-version">Training for Life {APP_VERSION}</p></div></details>
  </div>;
}
