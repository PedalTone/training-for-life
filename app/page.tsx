...
function HistoryView({ now, sessions, activeSchedule, scheduleHistory, onOpenDate }: { now: Date; sessions: Session[]; activeSchedule: Schedule; scheduleHistory: ScheduleSnapshot[]; onOpenDate: (date: Date) => void }) {
  ...
  const recentRecords = sessions.filter((saved) => saved.status !== "partial" || Boolean(saved.activity || saved.notes || saved.duration || saved.distance || saved.mobilityExercises.length || saved.completedExercises.length || hasReportedInjury(saved))).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 6);
  return <div className="subpage history-page">
    ...
    <section className="recent-records"><div className="recent-records-heading"><div><span className="kicker">RECENT RECORDS</span><h2>What you logged</h2></div><span>Tap a day to edit</span></div>{recentRecords.length ? <div className="recent-record-list">{recentRecords.map((saved) => { const date = dateFromKey(saved.date); const plan = historicalPlan(saved, scheduleForDate(date, activeSchedule, scheduleHistory), date); const state = stateFor(saved, plan.key, date, now); return <button key={saved.id} onClick={() => onOpenDate(date)}><span className={`recent-record-icon ${plan.key}`}>{plan.icon}</span><span className="recent-record-copy"><strong>{date.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })} · {plan.theme}</strong><small>{saved.activity || (saved.status === "rest" ? "Rest / Recovery" : saved.notes || "Workout recorded")}</small></span><span className={`recent-record-state ${state}`}>{hasReportedInjury(saved) ? "⚑ Injury" : stateLabel(state)}</span><i>›</i></button>; })}</div> : <p className="empty-state">Your completed workouts and notes will appear here.</p>}</section>
  </div>;
}
