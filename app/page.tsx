...
function PerformanceView({ now, sessions, activeSchedule, scheduleHistory, insightReports, setInsightReports, fitnessGoals, accessCode, onOpenSettings }: { now: Date; sessions: Session[]; activeSchedule: Schedule; scheduleHistory: ScheduleSnapshot[]; insightReports: TrainingInsightReport[]; setInsightReports: React.Dispatch<React.SetStateAction<TrainingInsightReport[]>>; fitnessGoals: FitnessGoals; accessCode: string; onOpenSettings: () => void }) {
  ...
  return <div className="subpage performance-page">
    ...
    <section className="performance-overview"><header className="performance-overview-hero"><div><span className="kicker">LAST 30 DAYS</span><strong>{adherence}% on rhythm</strong><p>{last30.length ? `${last30.length} recorded ${last30.length === 1 ? "day" : "days"} in this window.` : "Record a workout to begin seeing your pattern."}</p></div><span className="performance-arrow">↗</span></header><div className="performance-overview-metrics"><div><span>Current rhythm</span><strong>{currentStreak}<small> {currentStreak === 1 ? "day" : "days"}</small></strong></div><div><span>Consistent weeks</span><strong>{consistentWeeks}<small> of 8</small></strong></div><div><span>Recorded</span><strong>{sessions.length}<small> {sessions.length === 1 ? "day" : "days"}</small></strong></div></div><div className="performance-overview-week"><div className="card-heading"><div><span className="kicker">THIS WEEK</span><h2>Plan vs. record</h2></div><strong>{weekRecorded} / {week.length}</strong></div><p>Use Plan to look ahead; this is the quick performance readout.</p><div className="performance-bar"><i style={{ width: `${Math.round(weekRecorded / Math.max(1, week.length) * 100)}%` }}/></div><div className="performance-foot"><span>{injuryDays} body {injuryDays === 1 ? "consideration" : "considerations"} in history</span><span>{last30.filter((item) => sessionState(item) === "completed").length} completed in 30 days</span></div></div></section>
